...
def load_model(
    ...
    if model_args != "from_config" and type(model_args) != dict:
        raise ValueError(
            'model_args must either be "from_config" or a dictionary of values'
        )
    ...
    for i_dataset, dataset in enumerate(datasets):
        ...
        with torch.no_grad():
            ...
            for i_all, (data_x, data_y) in enumerate(loader):
                data_x = [
                    v.cuda(non_blocking=True) if v is not None else v for v in data_x
                ]
                data_y = [
                    v.cuda(non_blocking=True) if v is not None else v for v in data_y
                ]
...
def get_cached_pascal_pfe(split, config):
    ...
    try:
        dataset = DATASET_CACHE[
            (split, config.image_size, config.label_support, config.mask)
        ]
    except KeyError:
        dataset = PFEPascalWrapper(
            ...
            label_support=config.label_support,
        )
        DATASET_CACHE[
            (split, config.image_size, config.label_support, config.mask)
        ] = dataset
...
def score(config, train_checkpoint_id, train_config):
    ...
    train_config = AttributeDict(
        json.load(open(f"logs/{train_checkpoint_id}/config.json"))
    )
    ...
    _, model_args, _ = filter_args(
        train_config, inspect.signature(model_cls).parameters
    )

    model_args = {
        **model_args,
        **{k: config[k] for k in ["process_cond", "fix_shift"] if k in config},
    }

    strict_models = {"ConditionBase4", "PFENetWrapper"}
    model = load_model(
        train_checkpoint_id,
        strict=model_cls.__name__ in strict_models,
    ...
    if config.test_dataset == "pascal":
        ...
        for split in splits:
            ...
            metric = FixedIntervalMetrics(
                resize_pred=True, sigmoid=True, custom_threshold=shift, **metric_args
            )
            ...
            with torch.no_grad():
                ...
                for i_all, (data_x, data_y) in enumerate(loader):
                    data_x = [
                        v.cuda(non_blocking=True) if isinstance(v, torch.Tensor) else v
                        for v in data_x
                    ]
                    data_y = [
                        v.cuda(non_blocking=True) if isinstance(v, torch.Tensor) else v
                    ...
                    if config.mask == "separate":  # for old CondBase model
                        ...
                        pred, _, _, _ = model(
                            data_x[0], data_x[1], return_features=True
                        )
            ...
            log.info(
                f"Dataset length: {len(dataset)}, took {time.time() - eval_start_t:.1f}s to evaluate."
            )
        ...
        valid_keys = [
            ...
            for k in all_keys
            if all(
                v[k] is not None and isinstance(v[k], (int, float, np.float))
        ...
        return {
            key_prefix: {
                k: np.mean([s[k] for s in scores.values()]) for k in valid_keys
            }
        }
    ...
    if config.test_dataset == "pascal_zs":
        ...
        print(
            ...
            [
                c
                for class_set in PASCAL_VOC_CLASSES_ZS[: n_unseen // 2]
                for c in class_set
            ],
        )
        ...
        for i, (data_x, data_y) in enumerate(loader):
            ...
            evaluator.add_batch(
                data_y[0].numpy(), pred.argmax(1).cpu().detach().numpy()
            )
        ...
        return {
            key_prefix: {
                k: scores[k] for k in ["seen", "unseen", "harmonic", "overall"]
            }
        }
        ...
        _, dataset_args, _ = filter_args(
            config, inspect.signature(dataset_cls).parameters
        )

        dataset_args[
            "image_size"
        ] = train_config.image_size  # explicitly use training image size for evaluation
        ...
        with torch.no_grad():  # TODO: switch to inference_mode (torch 1.9)
            ...
            for data_x, data_y in data_loader:
                data_x = [
                    x.cuda() if isinstance(x, torch.Tensor) else x for x in data_x
                ]
                data_y = [
                    x.cuda() if isinstance(x, torch.Tensor) else x for x in data_y
                ]
        ...
        name_neg = (
            "" if dataset.negative_prob == 0 else "_" + str(dataset.negative_prob)
        )

        score_name = (
            config.name if "name" in config else f"{dataset_name}_{name_mask}{name_neg}"
        )
